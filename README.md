...
## Manual verification checklist (needs a logged-in client)

The unit suite covers all roll math, state, and data. These need human eyes in game:
...
3. Chest ceremony (all four tiers + a jackpot upgrade; Rusty: muted ceremony, retires
   after 3 opens, pity bar must not move), card flips, hover charge glow, shiny/hologram
   effects, stardust near-miss fizzle, pity break, in-reveal token reroll.
...
9. Party roll: proposal + agreement (busy members auto-excused), identical offers on all
   screens, unanimous vote accepts, pooled kill count syncs, carry clause on party leaving.
